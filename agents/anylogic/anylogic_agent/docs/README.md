...
For this type of simulation, you must use the template given to develop the simulation and import the `BirdgeConnection` agent, which should be placed inside the `Main` agent. The `BridgeConnection` handles the UDP connection and communication with the AnyLogic agent.
