...
class MatlabStreamingController:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            sim_name: Name of the simulation for result identification
        """
        ...
        self.host = tcp_settings.get('host', 'localhost')
        self.port = tcp_settings.get('port', 5678)
    ...
    def start(self) -> None:
        ...
        try:
            ...
            progress_template = response_templates.get('progress', {})
...
def create_response(template_type: str, sim_name: str, **kwargs) -> Dict[str, Any]:
    """
    ...
    timestamp_format = template.get('timestamp_format', '%Y-%m-%dT%H:%M:%SZ')
...
def handle_streaming_simulation(parsed_data: dict, source: str, rabbitmq_manager: RabbitMQManager) -> None:
    """
    ...
    logger.info(f"Processing streaming simulation: {sim_name}")
    ...
    try:
        ...
        logger.info(f"Streaming simulation '{sim_name}' completed successfully")
        
    except Exception as e:
        logger.error(f"Streaming simulation '{sim_name}' failed: {str(e)}", exc_info=True)
