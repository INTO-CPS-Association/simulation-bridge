...
class Simul8Simulator:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self.events = client.WithEvents(self.s8, self._create_event_handler())
            
            logger.debug("Simul8 engine started successfully")
        except Exception as e:
            logger.error("Failed to start Simul8 engine: %s", str(e))
            self.cleanup()
            raise Simul8SimulationError(f"Failed to start Simul8 engine: {str(e)}") from e
    ...
    def run(self, file_path: Optional[str] = None, inputs: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        if self.s8 is None:
            self.start()
            ...
            if hasattr(self, 'actual_file_path') and self.actual_file_path:
                ...
                try:
                    ...
                    if config_sim_path and os.path.exists(config_sim_path):
                        ...
                        examples_dir = os.path.normpath(os.path.join(current_dir, "..", "..", "docs", "examples"))
    ...
    def _collect_simulation_results(self) -> None:
        ...
        try:
            ...
            if results:
                ...
                logger.debug(f"Collected {len(results)} results from output file")
            else:
