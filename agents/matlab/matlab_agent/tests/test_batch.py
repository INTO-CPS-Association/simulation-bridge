...
def test_matlab_simulator_init_valid(mock_config):
    with patch('pathlib.Path.exists') as mock_exists:
        ...
        assert simulator.sim_path == Path('matlab_agent/docs/examples').resolve()
...
def test_matlab_simulator_start_success(mock_start, mock_config):
    ...
    mock_engine.addpath.assert_called_with(str(Path('matlab_agent/docs/examples').resolve()), nargout=0)
...
def test_matlab_simulator_run_success():
    ...
    result = simulator.run({'x_i': 10, 'y_i': 10, 'z_i': 10, 'v_x': 1, 'v_y': 1, 'v_z': 1, 't': 10}, ['x_f', 'y_f', 'z_f'])
    
    assert result == {'x_f': 20.0, 'y_f': 20.0, 'z_f': 20.0}
    mock_engine.feval.assert_called_with('simulation_batch', 10.0, 10.0, 10.0, 1.0, 1.0, 1.0, 10.0, nargout=3)
