...
def test_config_manager_defaults(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setattr(ConfigManager, "__init__", lambda self, config_path=None: setattr(self, 'config', Config().model_dump()))
    cm = ConfigManager()
...
def test_matlab_agent_initialization(monkeypatch):
    ...
    monkeypatch.setattr('src.core.agent.ConfigManager', lambda self=None: type('CM', (), {'get_config': lambda s: fake_cfg})())
    monkeypatch.setattr('src.core.agent.RabbitMQManager', lambda agent_id, config: type('RM', (), {'register_message_handler': mock.Mock(), 'start_consuming': mock.Mock(), 'close': mock.Mock()})())
    monkeypatch.setattr('src.core.agent.MessageHandler', lambda agent_id, rm: type('MH', (), {'handle_message': mock.Mock()})())
