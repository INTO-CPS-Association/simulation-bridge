...
Client RabbitMQ asincrono per inviare richieste di simulazione interattive
verso MATLAB e gestire lo streaming input/output con aio-pika e asyncio.
"""
...
class AsyncInteractiveMatlabClient:
    ...
    async def setup(self):
        ...
        self.ex_bridge = await self.channel.declare_exchange("ex.bridge.output", ExchangeType.TOPIC, durable=True)
        self.ex_result = await self.channel.declare_exchange("ex.sim.result", ExchangeType.TOPIC, durable=True)
        self.ex_stream = await self.channel.declare_exchange("ex.input.stream", ExchangeType.TOPIC, durable=True)

        self.queue = await self.channel.declare_queue(self.result_queue, durable=True)
        await self.queue.bind(self.ex_result, routing_key=f"{self.destination_id}.result.{self.agent_id}")

    async def send_initial_interactive_request(self, payload: Dict[str, Any], request_id: str):
    ...
    async def stream_inputs(self, request_id: str, stream_key: str):
        ...
        for t in range(100):
            ...
            print(f"[STREAM] Sent frame t={t * 0.1}")
...
async def main():
    ...
    parser.add_argument("--api-payload", type=str, default="../api/simulation.yaml")
    args = parser.parse_args()

    with open(args.api_payload, "r", encoding="utf-8") as f:
    ...
    # Avvia listener e streaming contemporaneamente
