...
To start the agent with the default configuration (`agent_id=matlab`), navigate to the root directory of the project and run the following command:

```bash
poetry run matlab-agent
```

To customize settings such as `agent_id`, edit the configuration file before starting the agent.
